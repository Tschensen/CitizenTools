...
def main():
    ...
    def check():
        ...
        try:
            ...
            expect("document.querySelector('[data-document=\"CITIZEN-TOOLS-NOTICE.txt\"] pre').textContent.includes('CitizenTools-Solo-OCR-Sources.zip')", 'third-party-scope-and-ocr-sources-disclosed')
            ...
            expect(f"document.querySelector('[data-release]').textContent.includes({json.dumps(solo_about.releases()[0]['en']['title'])}) && soloAboutContent.textContent.includes('This version')", 'english-release-notes')
